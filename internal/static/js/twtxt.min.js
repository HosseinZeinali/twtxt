...
var Twix=(function(){function Twix(){}
...
return Twix.ajax({url:url,data:data,type:type,success:callback});};Twix.get=function(url,data,callback){return _ajax("GET",url,data,callback);};Twix.head=function(url,data,callback){return _ajax("HEAD",url,data,callback);};Twix.post=function(url,data,callback){return _ajax("POST",url,data,callback);};Twix.patch=function(url,data,callback){return _ajax("PATCH",url,data,callback);};Twix.put=function(url,data,callback){return _ajax("PUT",url,data,callback);};Twix.delete=function(url,data,callback){return _ajax("DELETE",url,data,callback);};Twix.options=function(url,data,callback){return _ajax("OPTIONS",url,data,callback);};return Twix;})();__=Twix;/*!
* Theme Switcher
*
* Pico.css - https://picocss.com
* Copyright 2019 - Licensed under MIT
*/(function(){var switcher={button:{element:'BUTTON',class:'contrast switcher theme-switcher',on:'<i>Turn on dark mode</i>',off:'<i>Turn off dark mode</i>'},target:'body',selector:'button.theme-switcher',currentTheme:systemColorScheme()};themeSwitcher();function systemColorScheme(){let theme=getCookie("theme");switch(theme){case "light":case "dark":return theme;}
if(window.matchMedia('(prefers-color-scheme: dark)').matches){return 'dark';}
else{return 'light';}}
function themeSwitcher(){var button=document.createElement(switcher.button.element);button.className=switcher.button.class;document.querySelector(switcher.target).appendChild(button);setTheme(switcher.currentTheme);var switchers=document.querySelectorAll(switcher.selector);for(var i=0;i<switchers.length;i++){switchers[i].addEventListener('click',function(event){if(switcher.currentTheme=='light'){setTheme('dark');storeTheme('dark');}
else{setTheme('light');storeTheme('light');}},false);}}
function getCookie(cname){var name=cname+"=";var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++){var c=ca[i];while(c.charAt(0)==' '){c=c.substring(1);}
if(c.indexOf(name)==0){return c.substring(name.length,c.length);}}
return "";}
function setTheme(set){if(set=='light'){var label=switcher.button.on;}
else{var label=switcher.button.off;}
document.querySelector('html').setAttribute('data-theme',set);var switchers=document.querySelectorAll(switcher.selector);for(var i=0;i<switchers.length;i++){switchers[i].innerHTML=label;switchers[i].setAttribute('aria-label',stripTags(label));}
switcher.currentTheme=set;}
function storeTheme(set){document.cookie="theme="+set+"; expires=Fri, 31 Dec 9999 23:59:59 UTC; path=/";}
function stripTags(html){return html.replace(/<[^>]*>?/gm,'');}})();var dropCookie=true;var cookieDuration=14;var cookieName='complianceCookie';var cookieValue='on';var $mentionedList=u("#mentioned-list").first()
var lastSymbol=''
...
function replyTo(e){e.preventDefault();var el=u("textarea#text")
var text=document.getElementById("text");el.empty();el.text(u(e.target).data("reply"));el.scroll();text.focus();var size=el.text().length;text.setSelectionRange(size,size);}
function editTwt(e){e.preventDefault();var el=u("textarea#text")
var text=document.getElementById("text");el.empty();el.text(u(e.target).data("text"));el.scroll();text.focus();var size=el.text().length;text.setSelectionRange(size,size);u("#replaceTwt").first().value=u(e.target).data("hash");}
function deleteTwt(e){e.preventDefault();if(confirm("Are you sure you want to delete this twt? This cannot be undone!")){Twix.ajax({type:"DELETE",url:u("#twtForm").attr("action"),success:function(data){var hash=u(e.target).data("hash");u("#"+hash).remove();}});}};u(".reply").on("click",replyTo);u(".edit").on("click",editTwt);u(".delete").on("click",deleteTwt);u("#post").on("click",function(e){e.preventDefault();u("#post").html("<i class=\"icss-spinner icss-pulse\"></i>&nbsp;Posting...");u("#post").attr("disabled",true);u("#twtForm").first().submit();});u.prototype.getSelection=function(){var e=this.first();return(('selectionStart'in e&&function(){var l=e.selectionEnd-e.selectionStart;return{start:e.selectionStart,end:e.selectionEnd,length:l,text:e.value.substr(e.selectionStart,l)};})||(document.selection&&function(){e.focus();var r=document.selection.createRange();if(r===null){return{start:0,end:e.value.length,length:0}}
var re=e.createTextRange();var rc=re.duplicate();re.moveToBookmark(r.getBookmark());rc.setEndPoint('EndToStart',re);return{start:rc.text.length,end:rc.text.length+r.text.length,length:r.text.length,text:r.text};})||function(){return null;})();}
u.prototype.replaceSelection=function(){var e=this.first();var text=arguments[0]||'';return(('selectionStart'in e&&function(){e.value=e.value.substr(0,e.selectionStart)+text+e.value.substr(e.selectionEnd,e.value.length);return this;})||(document.selection&&function(){e.focus();document.selection.createRange().text=text;return this;})||function(){e.value+=text;return jQuery(e);})();}
function createMentionedUserNode(username){return u("<div>").addClass("user-list__user").append(u("<div>").addClass("avatar").attr("style","background-image: url('/user/"+username+"/avatar.png')")).append(u("<div>").addClass("info").append(u("<div>").addClass("nickname").text(username)));}
function formatText(selector,fmt){var finalText="";var selectedText=selector.getSelection().text;if(selectedText.length==0){finalText=fmt+fmt;}else{finalText=fmt+selectedText+fmt;}
selector.replaceSelection(finalText,true);selector.first().focus();if(!selectedText.length){var selectionRange=selector.first().value.length-fmt.length
selector.first().setSelectionRange(selectionRange,selectionRange)}}
function insertText(selector,text){selector.replaceSelection(text,true);selector.first().focus();selector.first().setSelectionRange(-1,-1);var selectorLength=selector.first().value.length;var selectionRange=selector.first().value.substr(-1)===')'?selectorLength-1:selectorLength;selector.first().setSelectionRange(selectionRange,selectionRange)}
function iOS(){return['iPad Simulator','iPhone Simulator','iPod Simulator','iPad','iPhone','iPod'].includes(navigator.platform)||(navigator.userAgent.includes("Mac")&&"ontouchend"in document)}
var deBounce=300
var fetchUsersTimeout=null
function getUsers(searchStr){clearTimeout(fetchUsersTimeout)
fetchUsersTimeout=setTimeout(function(){let requestUrl='/lookup';if(searchStr){requestUrl+='?prefix='+searchStr;}
Twix.ajax({type:"GET",url:requestUrl,success:function(data){u("#mentioned-list-content").empty();data.map(function(name){u('#mentioned-list-content').append(createMentionedUserNode(name));});}});},deBounce);}
function getLastMentionIndex(value){var regex=/@/gi,result,indices=[];while((result=regex.exec(value))){indices.push(result.index);}
return indices.slice(-1)[0]+1;}
u('#bBtn').on("click",function(e){e.preventDefault();formatText(u("textarea#text"),"**");});u('#iBtn').on("click",function(e){e.preventDefault();formatText(u("textarea#text"),"*");});u('#sBtn').on("click",function(e){e.preventDefault();formatText(u("textarea#text"),"~~");});u('#cBtn').on("click",function(e){e.preventDefault();formatText(u("textarea#text"),"`");});u('#lnkBtn').on("click",function(e){e.preventDefault();insertText(u("textarea#text"),"[title](https://)");});u('#imgBtn').on("click",function(e){e.preventDefault();insertText(u("textarea#text"),"![](https://)");});u('#usrBtn').on("click",function(e){e.preventDefault();if(!$mentionedList.classList.contains('show')){insertText(u("textarea#text"),"@");lastSymbol=u("textarea#text").first().value.slice(-1);if(iOS()){u("#mentioned-list").first().style.top=u("textarea#text").first().clientHeight+2+'px';u("#mentioned-list").first().classList.add('show');getUsers();}}else{$mentionedList.classList.remove('show');}})
u("textarea#text").on("focus",function(e){if(e.relatedTarget===u('#usrBtn').first()){u("#mentioned-list").first().style.top=u("textarea#text").first().clientHeight+2+'px';u("#mentioned-list").first().classList.add('show');getUsers();}})
u("textarea#text").on("input",function(e){var value=u("textarea#text").first().value;if($mentionedList.classList.contains('show')){var lastIndex=getLastMentionIndex(value);if(e.inputType==='deleteContentBackward'&&lastSymbol==='@'){u("textarea#text").first().value=u("textarea#text").first().value.slice(lastIndex-1);$mentionedList.classList.remove('show');}else{var searchStr=value.slice(lastIndex)
if(searchStr&&searchStr!=='@'){getUsers(searchStr);}else{getUsers()}}}else{if(e.target.value.slice(-1)==='@'){$mentionedList.classList.add('show');u("#mentioned-list").first().style.top=u("textarea#text").first().clientHeight+2+'px';getUsers();}}
lastSymbol=value.slice(-1);})
u("body").on('keyup',function(e){if(e.keyCode===9&&$mentionedList.classList.contains('show')){var value=u("textarea#text").first().value;if(u(".mentioned-list-content").length){var lastIndex=getLastMentionIndex(value);u("textarea#text").first().value=value.slice(0,lastIndex);insertText(u("textarea#text"),u(".mentioned-list-content .user-list__user").nodes[0].innerText.trim());$mentionedList.classList.remove('show');}}})
u("#mentioned-list").on('click',function(e){var value=u("textarea#text").first().value;var lastIndex=getLastMentionIndex(value);u("textarea#text").first().value=value.slice(0,lastIndex);insertText(u("textarea#text"),e.target.innerText.trim());u("#mentioned-list").first().classList.remove('show');})
u('#uploadMedia').on("change",function(e){u("#uploadMediaButton").removeClass("icss-camera");u("#uploadMediaButton").addClass("icss-spinner icss-pulse");u("#uploadMedia").html("<i class=\"icss-spinner icss-pulse\"></i>");Twix.ajax({type:"POST",url:u("#uploadForm").attr("action"),data:new FormData(u("#uploadForm").first()),success:function(data){var el=u("textarea#text")
var text=document.getElementById("text");text.value+=" ![]("+data.Path+") ";el.scroll();text.focus();var size=el.text().length;text.setSelectionRange(size,size);u("#uploadMediaButton").removeClass("icss-spinner icss-pulse");u("#uploadMediaButton").addClass("icss-camera");},error:function(statusCode,statusText){u("#uploadMediaButton").removeClass("icss-spinner icss-pulse");u("#uploadMediaButton").addClass("icss-camera");alert("An error occurred uploading your media: "+statusCode+" "+statusText);}});});u('#burgerMenu').on("click",function(e){e.preventDefault();if(u('#mainNav').hasClass('responsive')){u('#mainNav').removeClass('responsive');}
else{u('#mainNav').addClass('responsive');}});/*!
* Theme Switcher
*
* Pico.css - https://picocss.com
* Copyright 2019 - Licensed under MIT
...
switcher.currentTheme=set;}
function storeTheme(set){document.cookie="theme="+set+"; expires=Fri, 31 Dec 9999 23:59:59 UTC; path=/";}
function stripTags(html){return html.replace(/<[^>]*>?/gm,'');}})();var Twix=(function(){function Twix(){}
Twix.ajax=function(options){options=options||{url:""};options.type=options.type.toUpperCase()||'GET';options.headers=options.headers||{};options.timeout=parseInt(options.timeout)||0;options.success=options.success||function(){};options.error=options.error||function(){};options.async=typeof options.async==='undefined'?true:options.async;var client=new XMLHttpRequest();if(options.timeout>0){client.timeout=options.timeout;client.ontimeout=function(){options.error('timeout','timeout',client);};}
client.open(options.type,options.url,options.async);for(var i in options.headers){if(options.headers.hasOwnProperty(i)){client.setRequestHeader(i,options.headers[i]);}}
client.send(options.data);client.onreadystatechange=function(){if(this.readyState==4&&((this.status>=200&&this.status<300)||this.status==304)){var data=this.responseText;var contentType=this.getResponseHeader('Content-Type');if(contentType&&contentType.match(/json/)){data=JSON.parse(this.responseText);}
options.success(data,this.statusText,this);}else if(this.readyState==4){options.error(this.status,this.statusText,this);}};if(options.async==false){if(client.readyState==4&&((client.status>=200&&client.status<300)||client.status==304)){options.success(client.responseText,client);}else if(client.readyState==4){options.error(client.status,client.statusText,client);}}
return client;};var _ajax=function(type,url,data,callback){if(typeof data==="function"){callback=data;data=undefined;}
return Twix.ajax({url:url,data:data,type:type,success:callback});};Twix.get=function(url,data,callback){return _ajax("GET",url,data,callback);};Twix.head=function(url,data,callback){return _ajax("HEAD",url,data,callback);};Twix.post=function(url,data,callback){return _ajax("POST",url,data,callback);};Twix.patch=function(url,data,callback){return _ajax("PATCH",url,data,callback);};Twix.put=function(url,data,callback){return _ajax("PUT",url,data,callback);};Twix.delete=function(url,data,callback){return _ajax("DELETE",url,data,callback);};Twix.options=function(url,data,callback){return _ajax("OPTIONS",url,data,callback);};return Twix;})();__=Twix;var dropCookie=true;var cookieDuration=14;var cookieName='complianceCookie';var cookieValue='on';var $mentionedList=u("#mentioned-list").first()
var lastSymbol=''
function createDiv(){u("body").prepend('<div id="cookie-law" class="container-fluid"><p>This website uses cookies. By continuing we assume your permission to deploy cookies, as detailed in our <a href="/privacy" rel="nofollow" title="Privacy Policy">privacy policy</a>. <a role="button" href="javascript:void(0);" onclick="removeMe();">Close</a></p></div>');createCookie(window.cookieName,window.cookieValue,window.cookieDuration);}
function createCookie(name,value,days){if(days){var date=new Date();date.setTime(date.getTime()+(days*24*60*60*1000));var expires="; expires="+date.toGMTString();}
else var expires="";if(window.dropCookie){document.cookie=name+"="+value+expires+"; path=/";}}
...
u('#uploadMedia').on("change",function(e){u("#uploadMediaButton").removeClass("icss-camera");u("#uploadMediaButton").addClass("icss-spinner icss-pulse");u("#uploadMedia").html("<i class=\"icss-spinner icss-pulse\"></i>");Twix.ajax({type:"POST",url:u("#uploadForm").attr("action"),data:new FormData(u("#uploadForm").first()),success:function(data){var el=u("textarea#text")
var text=document.getElementById("text");text.value+=" ![]("+data.Path+") ";el.scroll();text.focus();var size=el.text().length;text.setSelectionRange(size,size);u("#uploadMediaButton").removeClass("icss-spinner icss-pulse");u("#uploadMediaButton").addClass("icss-camera");},error:function(statusCode,statusText){u("#uploadMediaButton").removeClass("icss-spinner icss-pulse");u("#uploadMediaButton").addClass("icss-camera");alert("An error occurred uploading your media: "+statusCode+" "+statusText);}});});u('#burgerMenu').on("click",function(e){e.preventDefault();if(u('#mainNav').hasClass('responsive')){u('#mainNav').removeClass('responsive');}
else{u('#mainNav').addClass('responsive');}});var u=function(parameter,context){if(!(this instanceof u)){return new u(parameter,context);}
if(parameter instanceof u){return parameter;}
if(typeof parameter==='string'){parameter=this.select(parameter,context);}
if(parameter&&parameter.nodeName){parameter=[parameter];}
this.nodes=this.slice(parameter);};u.prototype={get length(){return this.nodes.length;}};u.prototype.nodes=[];u.prototype.addClass=function(){return this.eacharg(arguments,function(el,name){el.classList.add(name);});};u.prototype.adjacent=function(html,data,callback){if(typeof data==='number'){if(data===0){data=[];}else{data=new Array(data).join().split(',').map(Number.call,Number);}}
